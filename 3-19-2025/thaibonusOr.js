<script>
  $(document).ready(function () {
    ...
    var countriesEligibleForCup = ["SG", "MY", "TH", "JP"];

    // Define groupindia groups
    var groupindia1 = ["BH", "IQ", "OM", "QA", "AE", "PS", "TN", "DZ", "MA"];
    ...
    $(
      ".group1, .monamaxcupgroup, .group2, .group3, .group4, .japangroup1, .malaysiagroup1, .thaigroup1, .thaigroup12, .vietnamgroup2, .chinagroup1, .chinagroup3, .spaingroup1, .indiagroup1, .indiagroup2, .indiagroup3, .indiagroup4"
    ).hide();
    ...
    function removeIrrelevantContent(countryCode) {
      ...
      if (countriesEligibleForCup.includes(countryCode)) {
        ...
        $(".thaigroup12").show();
        $(".thaigroup12 section.recurring").remove();
      } else if (countryCode === "CN" || countryCode === "MY") {
        $(".thaigroup1").show();
